...
Phase 3 is complete (see status note in section 8 and docs/PHASE3_SUMMARY.md). Start with Phase 4: validation. Out-of-sample prediction results with genuinely held-out future weeks this time (Phase 2's 5-fold in-season CV was a first look, not this); a comparison against historical closing point spreads (available from roughly 1979 on - not yet acquired, see section 7); an adversarial audit of teams whose ratings conflict with historical evidence, each with an explanation or a fix. This phase locks the model family for good, per section 8. Three small open items, none blocking: (1) reconstruct playoff round labels for 1970-1998 games, currently just a generic PLAYOFF flag (needed by Phase 5, not Phase 4); (2) build a team-season conference/division table (also Phase 5, for CSI); (3) src/models/bradley_terry.py's REG_STRENGTH constant was chosen by grid search against the Phase 2 cross-validation protocol specifically - recheck it if Phase 4 changes that protocol before relying on the comparison.
