...
Phase 2 is complete (see status note in section 8 and docs/PHASE2_SUMMARY.md). Start with Phase 3: era normalization - the standardization layer (z-scores within season) and the era handling rules in section 6 (schedule-length normalization, 1982/1987 strikes, expansion/realignment), with a sensitivity report showing how much each rule moves rankings. Two open items from Phase 1 are still outstanding and still not blocking Phase 3: (1) reconstruct playoff round labels for 1970-1998 games, currently just a generic PLAYOFF flag; (2) build a team-season conference/division table (needed by Phase 5, not Phase 3). A third, smaller item from Phase 2: src/models/bradley_terry.py's REG_STRENGTH constant was chosen by grid search against the Phase 2 cross-validation protocol specifically - worth rechecking if Phase 4 changes that protocol.
