...
link and image resolves and every image has alt text - the same two
checks that caught live bugs on the sibling site."""
import re
...
def test_every_page_has_a_title_and_one_h1(site):
    for page in site.rglob("*.html"):
        html = page.read_text(encoding="utf-8")
        assert re.search(r"<title>[^<]+ · Home Field Advantage</title>", html), page.name
